...
def approval():
  ...
  try:
    ...
    if id in httpCaht:
      ...
      tks = ('Hello%20Sir%20!%2I%20Want%20To%20Buy%20This%20Tools%20My%20Key%20:%20'+id);os.system('xdg-open https://wa.me/+639098660941?text='+tks)
      time.sleep(1)
      exit()
  except:
    sys.exit()
...
def ckx():
    uuid = str(os.geteuid()) + str(os.getlogin())
    id = "-".join(uuid)
    server = requests.get(f'https://github.com/taxs-404/approval/blob/main/approval.txt').text
    try:
        httpCaht = requests.get(f"https://github.com/taxs-404/approval/blob/main/approval.txt").text
        if id in httpCaht:
            msg = str(os.geteuid())
            #pass
        else:
